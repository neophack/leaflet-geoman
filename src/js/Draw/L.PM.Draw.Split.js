  ...
  Draw.Split = Draw.Line.extend({
    ...
    _finishShape() {
      ...
      if (coords.length == 2) {
        coords.push([coords[1].lat,coords[1].lng+0.00000001]);
      }
      ...
      layers.forEach((l) => {
        ...
        if (l instanceof L.Polygon) {
          ...
          if (l instanceof L.Polygon) {
            ...
            const diff = multiPolygonToPolygons(this._cutLayer(layer, newLayer));
            const inter = this._intersectLayer(layer, newLayer);

            var polys = []
            var newpolys = [];

              for (var i = 0; i < diff.features.length; i++) {
                polys.push(diff.features[i]);
                newpolys.push(diff.features[i]);
              }

            // polys.push(inter);
            // console.log(polys, layer);
            var replaceflag=false;
            for (var i = 0; i < polys.length; i++) {
              let newpoly = mergePolygonsWithLine(polys[i], inter, feature({
                type: 'LineString',
                coordinates: [[[layer._latlngs[0][0].lng, layer._latlngs[0][0].lat], [layer._latlngs[0][layer._latlngs[0].length - 1].lng, layer._latlngs[0][layer._latlngs[0].length - 1].lat]]]
              }));
              if (newpoly) {
                replaceflag=true;
                newpolys[i] = newpoly;
            ...
            for (var i = 0; i < newpolys.length; i++) {
              ...
              let resultLayer = L.geoJSON(newpolys[i], l.options);
    ...
    _intersectLayer(layer, l) {
      const fg = L.geoJSON();
      let diff;
      // cut
      if (l instanceof L.Polygon) {
        // find layer difference
        diff = intersect(l.toGeoJSON(15), layer.toGeoJSON(15));
      } else {
        const features = flattenPolyline(l);
