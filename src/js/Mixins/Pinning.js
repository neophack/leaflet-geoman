...
const PinningMixin = {
  ...
  getLatLngsFromShape(shape) {
    // Check if the 'shape' parameter is provided, otherwise use default value 'e.getLatLngs()'
    var latLngs = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : shape.getLatLngs();

    // If the 'shape' is an instance of 'L.Polygon', create a new Polygon with the provided or default 'latLngs'
    // and return its LatLngs.
    if (shape instanceof L.Polygon) {
      return L.polygon(latLngs).getLatLngs();
    } else {
      // If it's not a Polygon, assume it's a Polyline.
      // Create a new Polyline with the provided or default 'latLngs' and return its LatLngs.
    ...
    this.relevantLayers.forEach(function (layer) {
      ...
      if (Object.keys(indexPaths).length > 0) {
        ...
        if (layer instanceof L.Polyline && layer.pm && !layer.pm.options.allowSelfIntersection) {
          pinnedVertex.layer.pm._coordsBeforeEdit = self.getLatLngsFromShape(pinnedVertex.layer, pinnedVertex.layer.getLatLngs());
