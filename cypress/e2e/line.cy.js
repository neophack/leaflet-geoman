describe('Draw & Edit Line', () => {
  ...
  it("doesn't snap to the vertex", () => {
    ...
    cy.window().then(({ map }) => {
      ...
      expect(layer.getLatLngs()[0].lat).to.be.closeTo(51.52538802368748, 1e-9);
      expect(layer.getLatLngs()[0].lng).to.be.closeTo(
        -0.15050450596240997,
        1e-9
      );
    });
    ...
    cy.window().then(({ map }) => {
      const layer = map.pm.getGeomanDrawLayers()[1];
      expect(layer.getLatLngs()[0].lat).to.be.closeTo(51.5258877375718, 1e-9);
      expect(layer.getLatLngs()[0].lng).to.be.closeTo(
        -0.15026355008465944,
        1e-9
      );
