describe('Draw Circle', () => {
  ...
  it('checks if radius is changed when outer marker is snapped', () => {
    ...
    cy.window().then(({ map }) => {
      ...
      expect(layer.getLatLng().lat).to.be.closeTo(51.51034504891232, 1e-9);
      expect(layer.getLatLng().lng).to.be.closeTo(-0.12428283691406251, 1e-9);
      expect(layer.getRadius()).to.be.closeTo(1240.3294565841613, 1e-6);
    });
  });
});
